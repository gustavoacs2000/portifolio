...
const fabVariants = {
  ...
  visible: { opacity: 1, scale: 1,   y: 0,  transition: { type: "spring" as const, stiffness: 300, damping: 22 } },
  exit:    { opacity: 0, scale: 0.8, y: 16, transition: { duration: 0.2 } },
};

const pulseRing = {
  initial: { scale: 1, opacity: 0.5 },
  animate: { scale: [1, 1.55, 1.55], opacity: [0.5, 0, 0], transition: { duration: 2.2, repeat: Infinity, ease: "easeOut" as const } },
};
              ...
              <motion.button onClick={() => setExpanded((v) => !v)}
                ...
                <AnimatePresence mode="wait">
                  {expanded
                    ...
                    : <motion.span key="wa" initial={{ rotate: 90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: -90, opacity: 0 }} transition={{ duration: 0.18 }} style={{ color: "white" }}><WhatsAppIcon size={24} /></motion.span>
                  }
                </AnimatePresence>
              </motion.button>
            </motion.div>
